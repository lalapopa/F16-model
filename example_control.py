...
def run_sim(x0, u0, max_episode=2000):
    ...
    # Control Define
    if CONST_STEP:
        stab_act = np.radians(utils_control.step_function(t0, dt, tn, 0, 20))
        throttle_act = utils_control.step_function(t0, dt, tn, 0, 0.5)
    else: 
        stab_act = utils_control.make_step_series(t0, dt, tn, np.radians(20), step_time=1, hold_time=1, bias=u0[0])
