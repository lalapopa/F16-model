...
def run_sim(env_config, model_name):
    ...
    while True:
        ...
        if done:
            ...
            internal_states.append(info["final_info"][0]["state"])
            break
        else:
            clock.append(info["clock"][0])
            states.append(F16.denormalize(state[0]))
            internal_states.append(info["state"][0])
...
if __name__ == "__main__":
    ...
    for i in range(1, 2):
        init_state = np.array([0, 2000, 0, 0, 275, 0])
        ...
        states, inside_state, actions, ref_signal, r, t = run_sim(
